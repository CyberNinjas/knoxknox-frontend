...
var createFolderGlobs = function(fileTypePatterns) {
  ...
  var baseDir = "app/";
  var ignore = ['bower_components','temp'];
  var fs = require('fs');
  return fs.readdirSync(process.cwd() + "/" + baseDir)
          .map(function(file){
            if (ignore.indexOf(file) !== -1 || file.indexOf('.') === 0 || !fs.lstatSync(baseDir + file).isDirectory()) {
                return null;
            } else {
                return fileTypePatterns.map(function(pattern) {
                    return baseDir + file + '/**/' + pattern;
                });
            }
          })
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      ...
      azure: {
          files: [
              {cwd: 'dist', src: ['**'], dest: '../wwwroot/', expand: true}
          ]
      }
    },
    dom_munger:{
  ...
  grunt.registerTask('buildAndDeploy', ['build', 's3']);
  grunt.registerTask('buildAndAzureDeploy', ['build', 'clean:azure', 'copy:azure']);
